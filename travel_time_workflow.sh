...
# Check the column names which need to be use in the following command. Make them the same for GRASS GIS 7.8.
...
./travel_time_flow_columns.py lines diameter_column=DIAMETER liquid_height_coefficient="0.25"
./travel_time_cost_surface.py input=lines elevation=elevation roughness_column=n hydraulic_radius_column=hydraulic_radius output=cost coordinates=2151499.4,717482.6

# The rest is optional. Useful for visualization.
./travel_time_visualization_helpers.py cost=cost network_buffer=network_buffer raster_choropleth=raster_choropleth vector_choropleth=vector_choropleth_detailed max_time=5 time_step=0.5
